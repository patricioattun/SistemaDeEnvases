...
public class InternalReLiquidaSueldos extends javax.swing.JInternalFrame {
   ...
   private static InternalReLiquidaSueldos instancia=null;
  
    public InternalReLiquidaSueldos() throws ClassNotFoundException, SQLException {
        initComponents();
    ...
    public static InternalReLiquidaSueldos getInstancia() {
        return instancia;
    }

    public static void setInstancia(InternalReLiquidaSueldos instancia) {
        InternalReLiquidaSueldos.instancia = instancia;
    }
    
    
     public static InternalReLiquidaSueldos instancia() throws ClassNotFoundException, SQLException
   {    
         if (instancia== null)
         {
            instancia = new InternalReLiquidaSueldos();
         }
         return instancia;
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setIconifiable(true);
        setTitle("Re Liquidacion de Sueldos");
    ...
    private void btnLiquidarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLiquidarActionPerformed
        try {
            ...
            this.persLiq = new PersistenciaLiquidacion(fecha,2);
            if(f!=null){
                ingresos=this.persLiq.verificoPrelacion(f);
                if(ingresos.isEmpty()){
                    this.persLiq.Reliquidar(f,fecha);
                ...
                if(ingresos.isEmpty()){
                    ArrayList<Funcionario> funcionarios = this.log.listadoFuncionariosActivosReLiq();
                    if(funcionarios!=null){
                        if(funcionarios.size()>0){
                            for(Funcionario k: funcionarios){
                                ...
                                InternalLiquidaSueldo.instancia().getTxtNombre().paint( InternalLiquidaSueldo.instancia().getTxtNombre().getGraphics());
                                InternalLiquidaSueldo.instancia().revalidate();
                                this.persLiq.Reliquidar(k,fecha);
                            }
                            InternalLiquidaSueldo.instancia().getTxtNombre().setText("Liquidación Finalizada");
                            InternalLiquidaSueldo.instancia().getTxtNumFunc().setText("");
                            InternalLiquidaSueldo.instancia().repaint();
    ...
    private void txtNumFuncKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtNumFuncKeyTyped
        ...
        if(evt.getKeyChar()==43){
            ...
            try {
                InternalListadoFuncFiltro listadoFunc=InternalListadoFuncFiltro.instancia(log,12);
    ...
    private void formInternalFrameClosed(javax.swing.event.InternalFrameEvent evt) {//GEN-FIRST:event_formInternalFrameClosed
       ...
       try {
           ...
           Logger.getLogger(InternalReLiquidaSueldos.class.getName()).log(Level.SEVERE, null, ex);
       }

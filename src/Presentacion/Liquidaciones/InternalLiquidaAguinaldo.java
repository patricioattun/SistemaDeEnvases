...
public class InternalLiquidaAguinaldo extends javax.swing.JInternalFrame {
   ...
   private static InternalLiquidaAguinaldo instancia=null;
   
   public InternalLiquidaAguinaldo() throws ClassNotFoundException, SQLException {
        initComponents();
    ...
    public static InternalLiquidaAguinaldo getInstancia() {
        return instancia;
    }

    public static void setInstancia(InternalLiquidaAguinaldo instancia) {
        InternalLiquidaAguinaldo.instancia = instancia;
    }
    ...
    private void initComponents() {
        ...
        checkReLiq = new javax.swing.JCheckBox();

        setClosable(true);
        setIconifiable(true);
        setTitle("Liquidación y Re-Liquidación de Aguinaldo");
        ...
        checkReLiq.setFont(new java.awt.Font("Dialog", 1, 14)); // NOI18N
        checkReLiq.setText("Re-Liquidación");
        ...
        layout.setVerticalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(checkReLiq, javax.swing.GroupLayout.Alignment.TRAILING))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
    ...
    private void txtNumFuncFocusGained(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtNumFuncFocusGained
     ...
     public static InternalLiquidaAguinaldo instancia() throws ClassNotFoundException, SQLException
   {    
         if (instancia== null)
         {
            instancia = new InternalLiquidaAguinaldo();
         }
         return instancia;
    ...
    private void btnLiquidarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLiquidarActionPerformed
       try {
           ...
           boolean reLiq = this.checkReLiq.isSelected();
           Date fecha = this.stringADate(this.txtFechaLiq.getText());
           this.persLiq = new PersistenciaLiquidacion(fecha,1);
           if(this.esFechaAcorde(reLiq)){
           if(f!=null){
               ingresos=this.persLiq.verificoPrelacion(f);
               if(ingresos.isEmpty()){
                    this.persLiq.liquidar(f,fecha,5,reLiq);
                    
                        InternalLiquidaAguinaldo.instancia().getTxtNombre().setText("Liquidación Finalizada");
                        InternalLiquidaAguinaldo.instancia().getTxtNumFunc().setText("");
                        InternalLiquidaAguinaldo.instancia().repaint();
                        InternalLiquidaAguinaldo.instancia().getTxtNombre().paint(InternalLiquidaAguinaldo.instancia().getTxtNombre().getGraphics());
                        InternalLiquidaAguinaldo.instancia().getTxtNumFunc().paint(InternalLiquidaAguinaldo.instancia().getTxtNumFunc().getGraphics());
                        InternalLiquidaAguinaldo.instancia().revalidate();
                        f=null;
                    ...
                    if(funcionarios!=null){
                        if(funcionarios.size()>0){
                               for(Funcionario k: funcionarios){
                                    InternalLiquidaAguinaldo.instancia().getTxtNombre().setText(k.getNomCompletoApe());
                                    InternalLiquidaAguinaldo.instancia().getTxtNumFunc().setText(k.getCodFunc().toString());
                                    InternalLiquidaAguinaldo.instancia().repaint();
                                    InternalLiquidaAguinaldo.instancia().getTxtNumFunc().paint(InternalLiquidaAguinaldo.instancia().getTxtNumFunc().getGraphics());
                                    InternalLiquidaAguinaldo.instancia().getTxtNombre().paint(InternalLiquidaAguinaldo.instancia().getTxtNombre().getGraphics());
                                    InternalLiquidaAguinaldo.instancia().revalidate();
                                    this.persLiq.liquidar(k,fecha,5,reLiq);
                                }
                                    InternalLiquidaAguinaldo.instancia().getTxtNombre().setText("Liquidación Finalizada");
                                    InternalLiquidaAguinaldo.instancia().getTxtNumFunc().setText("");
                                    InternalLiquidaAguinaldo.instancia().repaint();
               ...
               JOptionPane.showMessageDialog(this, "La fecha de liquidación no coincide con el tipo de liquidación");
           }
            }else{
           ...
           this.lblMsj.setText(ex.getMessage());
            Logger.getLogger(InternalLiquidaAguinaldo.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private javax.swing.JCheckBox checkReLiq;
    ...
    private boolean esFechaAcorde(boolean reLiq) throws ParseException {
        Calendar cal = Calendar.getInstance();
        cal.setTime(this.stringADate(this.txtFechaLiq.getText())); 
        int dia=cal.get(Calendar.DAY_OF_MONTH);
        
        if(dia==15 && !reLiq){
            return true;
        }else return dia==14 && reLiq;
